...
def check_login():
    ...
    if (ENTRY_1.get().lower()=='zohaib') and (ENTRY_2.get().lower()=='xijinping090'):
        ...
        A_G_LABEL = Label(screen, text="***Access Granted***", font=('Arial 25 bold'), fg='green')
        A_G_LABEL.grid(row=0, column=0, padx=50, pady=160)
        screen.mainloop()
    else:
        src = Tk()
        src.geometry('400x400')
        src.configure(background='white')
        
        A_G_LABEL = Label(src, text="***Access Denied***", font=('Arial 25 bold'), fg='red')
        A_G_LABEL.grid(row=0, column=0, padx=50, pady=160)
        src.mainloop()
